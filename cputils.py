...
def findHostByNameOrAddress(apiCon, host):
    resultsList = []
    (response, auth_error) = apiCon.getServerList()
...
def findGroupByName(apiCon, gname):
    (response, auth_error) = apiCon.getServerGroupList()
...
def getFirewallPolicyList(apiCon):
    (response, auth_error) = apiCon.getFirewallPolicyList()
...
def findOrCreateFirewallServices(apiCon, desired_list):
    (response, auth_error) = getFirewallServiceList(apiCon)
    if ('firewall_services' in response):
        ...
        for svc in desired_list:
            ...
            if id == None:
                (response, auth_error) = createFirewallService(apiCon,svc)
...
def findOrCreateFirewallPolicy(apiCon, policyName, platform):
    ...
    if not (desiredFirewallPolicy):
        ...
        (response, auth_error) = apiCon.createFirewallPolicy(policy)
        if auth_error:
...
def createFirewallService(apiCon,svcObj):
    ...
    (data, auth_error) = apiCon.doPostRequest(url, apiCon.authToken, jsonData)
    if (data):
...
def getFirewallServiceList(apiCon):
    url = "%s:%d/%s/firewall_services/" % (apiCon.base_url, apiCon.port, apiCon.api_ver)
    (data, auth_error) = apiCon.doGetRequest(url, apiCon.authToken)
    if (data):
        return (json.loads(data), auth_error)
    else:
        return (None, auth_error)
